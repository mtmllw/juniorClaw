...
echo "=> Utilizing native CLI to permanently wildcard execution approvals..."
rm -f ./data/exec-approvals.json
docker compose run --rm openclaw-cli /bin/sh -c "npx openclaw approvals allowlist add '/*' && npx openclaw approvals allowlist add '**/*' && npx openclaw approvals allowlist add '*'"
