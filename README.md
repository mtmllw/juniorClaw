...
2. Place the downloaded `.pem` file directly into the project root folder. The `setup.sh` script will automatically locate it and securely copy it into the `data/` directory.
3. Inside your `.env` file, provide your App configuration:
   - `GH_APP_ID=your_app_id`
   - `GH_APP_INSTALLATION_ID=your_installation_id`
   - `GH_APP_PRIVATE_KEY_PATH=/root/.openclaw/github-app.pem` *(Keep this path exactly!)*
